...
'''debugging/TODO
- add different n comparison
- come up with research question: is off policy more data efficient than on policy in this example? 
'''
...
class TD():
    ...
    def train(self):
        ...
        for _ in range(self.epochs):
            ...
            # self.action_storage[0] = None
            while tau != T - 1:
                if t < T:
                    if self.type != 'on':
                        curr_pos += 1 # QUESTION: what is curr pos exceeds the index of the maze????????
                        self.state_storage[t+1] = self.maze[curr_pos]
                        self.reward_storage[t+1] = 1
                        # self.action_storage[t+1] = 'R'
                    else:
                        ...
                        # self.action_storage[t+1] = 'R' if action == 1 else 'L'
                    try:
    ...
    # written by chatGPT: plot the mean square error or learned values vs true value
...
class Experiment():
    ...
    def plot(self, window = 20):
        ...
        # average_on = np.convolve(average_on, np.ones(window)/window, mode='valid')
...
if __name__ == '__main__':
    ...
    # exp = Experiment(seeds, 0.2, 2, maze, 0.8, 0.8, 200)
    # exp.train()
    # exp.plot()

    # exp1 = Experiment(seeds, 0.2, 4, maze, 0.8, 0.8, 200)
    # exp1.train()
    # exp1.plot()

    # exp2 = Experiment(seeds, 0.2, 6, maze, 0.8, 0.8, 200)
    # exp2.train()
    # exp2.plot()

    exp3 = Experiment(seeds, 0.2, 8, maze, 0.8, 0.2, 10)
    exp3.train()
    exp3.plot()
